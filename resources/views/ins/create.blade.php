...
@endsection
@section('content')
<form action="{{url('in/save')}}" method="POST" enctype="multipart/form-data" 
    autocomplete="off">
            ...
            <div class="row">
                <div class="col-sm-6">
                    <div class="form-group row">
                        <label for="date_in" class="col-sm-4">Date In<span class="text-danger">*</span></label>
                        <div class="col-sm-8">
                            <input type="date" name='date_in' id="date_in" autocomplete="off"
...
<script>
    ...
    function save()
    {
        ...
        else if(trs.length<=0)
        {
            ...
            // save to database
            let data = {stock_in: master, items: items};
  ...
  function saveProduct()
  {
    ...
    // let type = $("#ptype").val();
     ...
     else
     {
         $.ajax({
             type: "POST",
             url: burl +"/inventory/in/product/save",
             data: {
                 name: name,
                 category_id: cat,
                 code: code,
             ...
             success: function (data) {
                 ...
                 $("#productModal").modal('hide');
                    var opt = "<option value='" + data.id + "' pcode='" + data.code + "' pname='" + data.name + "'>" + data.name + "</option>";
                    $("#item option:first-child").before(opt);
                    $('#item').val(data.id);
                    $("#item").trigger('chosen:updated');
                    $("#pname").val("");
                    $("#pcode").val("");
  ...
  // function search by po
  function search()
  {
    let po = $("#po_no").val();
    $.ajax({
        type: "GET",
        url: burl + "/inventory/poform/search/" + po,
        success: function(sms)
        {
            console.log(sms);
